...
语言识别: [`fast-langdetect`](https://github.com/LlmKira/fast-langdetect)
...
- [2025-09-18] 升级 fast_langdetect 到 1.0+

## v2.1.0

- [2025-02-21] 修改合并逻辑

  - 大幅减少存在假名的 `ja` 字符串被合并到附近 `zh` 中（纯汉字日文和中文混合的情况在还是存在误判）
  - 处理速度提升 6~7 倍（移除了 `wordfreq` 的使用）
  <details>
  ...
  ### 所有测试结果（执行 `python .\tests\split_acc.py` 获取）
...
**阶段 3**: 连接子字串基于语言识别，通过 [`fast-langdetect`](https://github.com/LlmKira/fast-langdetect) 和正则表达式 (基于规则)
...
- `TTS (Text-To-Speech)` 文字转语言模型往往无法处理多语种混合的文本, 目前的解决方案通常有以下 2 种:
  - 训练一个 TTS 模型可以同时发音多种语言（但多种语言的发音规则和语法不同，为了达到音色一致，该种方案成本往往偏高）
  - **(这个包)** 将文本中不同语言的文本切分, 之后使用不同的 TTS 模型进行生成
- 现存的自然语音处理（NLP）包 (如：`SpaCy`, `jieba`) 通常每 1 个模型只针对 **1 种** 语言处理（考虑到不同语言的语法、词汇特性）。所以在多语言的文本上，需要进行语言切分的预处理，如以下情况:
...
- [更新记录](#更新记录)
  ...
  - [v2.0.6](#v206)
    - [before](#before-1)
    - [after](#after-1)
    - [所有测试结果（执行 `python .\tests\split_acc.py` 获取）](#所有测试结果执行-python-testssplit_accpy-获取-1)
- [1. 💡 运作原理](#1--运作原理)
...
- 语言识别基于 [zafercavdar/fasttext-langdetect](https://github.com/zafercavdar/fasttext-langdetect)
